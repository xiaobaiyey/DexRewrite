...
namespace base {
    ...
    bool ReadFileToString(const std::string &path, std::string *content, bool follow_symlinks) {
        ...
        base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags)));
        if (fd == -1) {
            return false;
        }
    ...
    bool WriteStringToFile(const std::string &content, const std::string &path,
        ...
        base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags, mode)));
        if (fd == -1) {
            PLOG(ERROR) << "WriteStringToFile open failed";
            return false;
        }
        ...
        if (fchmod(fd, mode) == -1) {
            PLOG(ERROR) << "WriteStringToFile fchmod failed";
            return CleanUpAfterFailedWrite(path);
        }
        if (fchown(fd, owner, group) == -1) {
            PLOG(ERROR) << "WriteStringToFile fchown failed";
            return CleanUpAfterFailedWrite(path);
        }
        if (!WriteStringToFd(content, fd)) {
            PLOG(ERROR) << "WriteStringToFile write failed";
            return CleanUpAfterFailedWrite(path);
        }
        return true;
    }
    ...
    bool WriteStringToFile(const std::string &content, const std::string &path,
        ...
        int flags = O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC | O_BINARY |
                    (follow_symlinks ? 0 : O_NOFOLLOW);
        base::unique_fd fd(TEMP_FAILURE_RETRY(open(path.c_str(), flags, 0666)));
        if (fd == -1) {
            return false;
        }
    ...
    std::string GetExecutablePath() {
        ...
        std::string path;
        base::Readlink("/proc/self/exe", &path);
        return path;
...
#error unknown OS
    ...
    bool WriteDataToFile(uint8_t *data, size_t len, std::string tofile) {
