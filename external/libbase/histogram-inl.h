...
namespace base {
    ...
    inline void Histogram<Value>::AddValue(Value value) {
    ...
    template<class Value>
    inline void Histogram<Value>::AdjustAndAddValue(Value value) {
        AddValue(value / kAdjust);
    }

    template<class Value>
    inline Histogram<Value>::Histogram(const char *name)
            : kAdjust(0),
    ...
    template<class Value>
    inline size_t Histogram<Value>::FindBucket(Value val) const {
    ...
    template<class Value>
    inline void Histogram<Value>::Initialize() {
    ...
    template<class Value>
    inline size_t Histogram<Value>::GetBucketCount() const {
        return frequency_.size();
    }

    template<class Value>
    inline void Histogram<Value>::Reset() {
        sum_of_squares_ = 0;
        sample_size_ = 0;
        min_ = 0;
        sum_ = 0;
    ...
    template<class Value>
    inline Value Histogram<Value>::GetRange(size_t bucket_idx) const {
        DCHECK_LE(bucket_idx, GetBucketCount());
        return min_ + bucket_idx * bucket_width_;
    }

    template<class Value>
    inline double Histogram<Value>::Mean() const {
        DCHECK_GT(sample_size_, 0ull);
        return static_cast<double>(sum_) / static_cast<double>(sample_size_);
    }

    template<class Value>
    inline double Histogram<Value>::Variance() const {
        DCHECK_GT(sample_size_, 0ull);
