...
using namespace base;
    namespace base {
        ...
        void KernelLogger(base::LogId, base::LogSeverity severity,
                          const char *tag, const char *, unsigned int, const char *msg) {
            // clang-format off
            static constexpr int kLogSeverityToKernelLogLevel[] = {
                    [base::VERBOSE] = 7,              // KERN_DEBUG (there is no verbose kernel log
                    //             level)
                    [base::DEBUG] = 7,                // KERN_DEBUG
                    [base::INFO] = 6,                 // KERN_INFO
                    [base::WARNING] = 4,              // KERN_WARNING
                    [base::ERROR] = 3,                // KERN_ERROR
                    [base::FATAL_WITHOUT_ABORT] = 2,  // KERN_CRIT
                    [base::FATAL] = 2,                // KERN_CRIT
            };
            // clang-format on
            static_assert(arraysize(kLogSeverityToKernelLogLevel) == base::FATAL + 1,
        ...
        void LogdLogger::operator()(LogId id, LogSeverity severity, const char *tag,
            ...
            if (priority == ANDROID_LOG_FATAL) {
                __android_log_print(priority, tag, "%s:%u] %s", file, line,
                                    message);
            } else {
        ...
        void InitLogging(char *argv[], LogFunction &&logger, AbortFunction &&aborter) {
            ...
            for (size_t i = 0; i < specs.size(); ++i) {
                ...
                if (spec.size() == 3 && StartsWith(spec, "*:")) {
                    switch (spec[2]) {
                        ...
                        case 'w':
                            ...
                            continue;
                        case 'e':
                            gMinimumLogSeverity = base::LogSeverity::ERROR;
                            continue;
                        case 'f':
                ...
                LOG(DEBUG) << "unsupported '" << spec << "' in ANDROID_LOG_TAGS (" << tags
                           << ")";
            }
        }
...
// checks/logging in a function.
        ...
        void LogMessage::LogLine(const char *file, unsigned int line, LogId id, LogSeverity severity,
            ...
            if (tag == nullptr) {
                std::lock_guard<std::recursive_mutex> lock(TagLock());
                if (gDefaultTag == nullptr) {
                    gDefaultTag = new std::string("LOG::");
                }
#ifdef __ANDROID__
                Logger()(id, severity, gDefaultTag->c_str(), file, line, message);
#endif
                std::cout << gDefaultTag << ":" << file << ":" << line << " " << message << std::endl;
...
#ifdef ART_TARGET_ANDROID
#define INIT_LOGGING_DEFAULT_LOGGER base::LogdLogger()
#else
#define INIT_LOGGING_DEFAULT_LOGGER base::StderrLogger
#endif
        base::InitLogging(argv, INIT_LOGGING_DEFAULT_LOGGER,
...
#undef INIT_LOGGING_DEFAULT_LOGGER
    ...
    static_assert(arraysize(kLogSeverityToAndroidLogPriority) == ::base::FATAL + 1,
                  "Mismatch in size of kLogSeverityToAndroidLogPriority and values in LogSeverity");
#endif
    ...
    void LogHelper::LogLineLowStack(const char *file,
        ...
        static_assert(
                arraysize(kLogCharacters) == static_cast<size_t>(::base::FATAL) + 1,
