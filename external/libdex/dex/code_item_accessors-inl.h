...
inline void CodeItemInstructionAccessor::Init(const DexFile& dex_file,
  ...
  if (code_item != nullptr) {
    ...
    if (dex_file.IsCompactDexFile()) {
      Init(base::down_cast<const CompactDexFile::CodeItem&>(*code_item));
    } else {
      DCHECK(dex_file.IsStandardDexFile());
      Init(base::down_cast<const StandardDexFile::CodeItem&>(*code_item));
    }
  }
}
...
inline base::IterationRange<DexInstructionIterator> CodeItemInstructionAccessor::InstructionsFrom(
...
inline base::IterationRange<const dex::TryItem*> CodeItemDataAccessor::TryItems() const {
...
inline const dex::TryItem* CodeItemDataAccessor::FindTryItem(uint32_t try_dex_pc) const {
  base::IterationRange<const dex::TryItem*> try_items(TryItems());
...
inline const void* CodeItemDataAccessor::CodeItemDataEnd() const {
  ...
  const uint32_t handlers_size = base::DecodeUnsignedLeb128(&handler_data);
  // Manually read each handler.
  for (uint32_t i = 0; i < handlers_size; ++i) {
    int32_t uleb128_count = base::DecodeSignedLeb128(&handler_data) * 2;
    if (uleb128_count <= 0) {
      uleb128_count = -uleb128_count + 1;
    }
    for (int32_t j = 0; j < uleb128_count; ++j) {
      base::DecodeUnsignedLeb128(&handler_data);
    }
  }
...
inline void CodeItemDebugInfoAccessor::Init<CompactDexFile::CodeItem>(
  ...
  debug_info_offset_ = base::down_cast<const CompactDexFile*>(dex_file_)->GetDebugInfoOffset(
      dex_method_index);
...
inline void CodeItemDebugInfoAccessor::Init(const DexFile& dex_file,
  ...
  dex_file_ = &dex_file;
  if (dex_file.IsCompactDexFile()) {
    Init(base::down_cast<const CompactDexFile::CodeItem&>(*code_item), dex_method_index);
  } else {
    DCHECK(dex_file.IsStandardDexFile());
    Init(base::down_cast<const StandardDexFile::CodeItem&>(*code_item), dex_method_index);
  }
}
