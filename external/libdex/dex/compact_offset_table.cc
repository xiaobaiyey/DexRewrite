...
uint32_t CompactOffsetTable::Accessor::GetOffset(uint32_t index) const {
  ...
  ++block;
  bit_mask = (bit_mask << base::kBitsPerByte) | *block;
  ++block;
  ...
  size_t count = base::POPCOUNT(static_cast<uintptr_t>(bit_mask) << (base::kBitsPerIntPtrT - 1 - bit_index));
  DCHECK_GT(count, 0u);
  uint32_t current_offset = minimum_offset_;
  do {
    current_offset += base::DecodeUnsignedLeb128(&block);
    --count;
  } while (count > 0);
...
void CompactOffsetTable::Build(const std::vector<uint32_t>& offsets,
  ...
  *out_min_offset = std::numeric_limits<uint32_t>::max();
  ...
  while (block_start < offsets.size()) {
    ...
    // Write bit mask.
    out_data->push_back(static_cast<uint8_t>(bit_mask >> base::kBitsPerByte));
    ...
    for (size_t i = 0; i < block_size; ++i) {
      ...
      if (offset != 0u) {
        ...
        base::EncodeUnsignedLeb128(out_data, delta);
        prev_offset = offset;
      }
    }
