...
void CatchHandlerIterator::Init(const uint8_t* handler_data) {
  ...
  remaining_count_ = base::DecodeSignedLeb128(&current_data_);
...
void CatchHandlerIterator::Next() {
  if (remaining_count_ > 0) {
    handler_.type_idx_ = dex::TypeIndex(base::DecodeUnsignedLeb128(&current_data_));
    handler_.address_  = base::DecodeUnsignedLeb128(&current_data_);
    remaining_count_--;
    return;
  }
