...
template<> struct hash<libdex::dex::ProtoIndex> {
  size_t operator()(const libdex::dex::ProtoIndex& index) const {
    return hash<decltype(index.index_)>()(index.index_);
  }
};

template<> struct hash<libdex::dex::StringIndex> {
  size_t operator()(const libdex::dex::StringIndex& index) const {
    return hash<decltype(index.index_)>()(index.index_);
  }
};

template<> struct hash<libdex::dex::TypeIndex> {
  size_t operator()(const libdex::dex::TypeIndex& index) const {
    return hash<decltype(index.index_)>()(index.index_);
  }
};
