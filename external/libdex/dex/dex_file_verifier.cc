...
bool DexFileVerifier::CheckValidOffsetAndSize(uint32_t offset,
  ...
  if (alignment != 0 && !base::IsAlignedParam(offset, alignment)) {
...
#define DECODE_UNSIGNED_CHECKED_FROM(ptr, var)                        \
  uint32_t var;                                                       \
  if (!base::DecodeUnsignedLeb128Checked(&(ptr), begin_ + size_, &(var))) { \
...
#define DECODE_SIGNED_CHECKED_FROM(ptr, var)                        \
  int32_t var;                                                      \
  if (!base::DecodeSignedLeb128Checked(&(ptr), begin_ + size_, &(var))) { \
    ErrorStringPrintf("Read out of bounds");                        \
    return false;                                                   \
  }
...
bool DexFileVerifier::CheckClassDataItemField(uint32_t idx,
  ...
  if (class_type_index != my_class_index) {
    ErrorStringPrintf("Field's class index unexpected, %u vs %u",
...
bool DexFileVerifier::CheckOrder(const char* type_descr,
  ...
  if (UNLIKELY(curr_index < prev_index)) {
    ErrorStringPrintf("out-of-order %s indexes %d vs %d",
                      type_descr,
                      prev_index,
                      curr_index);
    return false;
  ...
  if (!*have_class) {
    *have_class = FindClassIndexAndDef(field->GetIndex(), true, class_type_index, class_def);
    if (!*have_class) {
      ...
      ErrorStringPrintf("could not find declaring class for %s index %d" ,
                        kTypeDescr,
                        field->GetIndex());
      return false;
    }
...
bool DexFileVerifier::CheckIntraCodeItem() {
  ...
  if (handlers_size < kAllocaMaxSize/sizeof(uint32_t)) {
    ...
    handler_offsets =
        base::AlignUp(reinterpret_cast<uint32_t*>(alloca((handlers_size + 1) * sizeof(uint32_t))),
                alignof(uint32_t[]));
  } else {
...
bool DexFileVerifier::CheckIntraHiddenapiClassData() {
  ...
  for (uint32_t i = 0; i < dex_file_->NumClassDefs(); ++i) {
    ...
    auto fn_member = [&](const ClassAccessor::BaseItem& member, const char* member_type) {
      ...
      if (!base::DecodeUnsignedLeb128Checked(&ptr_, data_end, &decoded_flags)) {
...
bool DexFileVerifier::CheckInterClassDefItem() {
  ...
  if (item->annotations_off_ != 0) {
    ...
    if (!base::IsAlignedParam(item->annotations_off_, 4)) {
...
static constexpr bool CheckAtMostOneOfPublicProtectedPrivate(uint32_t flags) {
  ...
  static_assert(base::IsPowerOfTwo(0), "0 not marked as power of two");
  static_assert(base::IsPowerOfTwo(kAccPublic), "kAccPublic not marked as power of two");
  static_assert(base::IsPowerOfTwo(kAccProtected), "kAccProtected not marked as power of two");
  static_assert(base::IsPowerOfTwo(kAccPrivate), "kAccPrivate not marked as power of two");
  return base::IsPowerOfTwo(flags & (kAccPublic | kAccProtected | kAccPrivate));
}
...
static std::string GetStringOrError(const uint8_t* const begin,
  ...
  uint32_t dummy;
  if (!base::DecodeUnsignedLeb128Checked(&ptr, begin + header->file_size_, &dummy)) {
    return "(error)";
  }
...
void DexFileVerifier::FindStringRangesForMethodNames() {
  ...
  auto get_string = [begin = begin_](const dex::StringId& id) {
    ...
    base::DecodeUnsignedLeb128(&str_data_ptr);
...
bool DexFileVerifier::CheckMethodAccessFlags(uint32_t method_index,
  ...
  if (((method_access_flags & kAccConstructor) != 0) && !is_constructor_by_name) {
    *error_msg =
        StringPrintf("Method %u(%s) is marked constructor, but doesn't match name",
                      method_index,
                      GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
    return false;
  }
  ...
  if (is_constructor_by_name) {
    ...
    if (is_static ^ is_clinit_by_name) {
      *error_msg = StringPrintf("Constructor %u(%s) is not flagged correctly wrt/ static.",
                                method_index,
                                GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
      if (dex_file_->SupportsDefaultMethods()) {
        return false;
  ...
  if (is_direct != expect_direct) {
    *error_msg = StringPrintf("Direct/virtual method %u(%s) not in expected list %d",
                              method_index,
                              GetMethodDescriptionOrError(begin_, header_, method_index).c_str(),
                              expect_direct);
    return false;
  ...
  if ((class_access_flags & kAccInterface) != 0) {
    ...
    if ((method_access_flags & desired_flags) == 0) {
      *error_msg = StringPrintf("Interface virtual method %u(%s) is not public",
          method_index,
          GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
      if (dex_file_->SupportsDefaultMethods()) {
        return false;
  ...
  if (!has_code) {
    ...
    if ((method_access_flags & (kAccNative | kAccAbstract)) == 0) {
      *error_msg = StringPrintf("Method %u(%s) has no code, but is not marked native or "
                                "abstract",
                                method_index,
                                GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
      return false;
    ...
    if ((method_access_flags & kAccAbstract) != 0) {
      ...
      if ((method_access_flags & kForbidden) != 0) {
        *error_msg = StringPrintf("Abstract method %u(%s) has disallowed access flags %x",
            method_index,
            GetMethodDescriptionOrError(begin_, header_, method_index).c_str(),
            method_access_flags);
        return false;
    ...
    if ((class_access_flags & kAccInterface) != 0) {
      ...
      if ((method_access_flags & (kAccPublic | kAccAbstract)) != (kAccPublic | kAccAbstract)) {
        *error_msg = StringPrintf("Interface method %u(%s) is not public and abstract",
            method_index,
            GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
        if (dex_file_->SupportsDefaultMethods()) {
          return false;
  ...
  if ((method_access_flags & (kAccNative | kAccAbstract)) != 0) {
    *error_msg = StringPrintf("Method %u(%s) has code, but is marked native or abstract",
                              method_index,
                              GetMethodDescriptionOrError(begin_, header_, method_index).c_str());
    return false;
  }
  ...
  if (constructor_flags_by_name == kAccConstructor) {
    ...
    if ((method_access_flags & ~kInitAllowed) != 0) {
      *error_msg = StringPrintf("Constructor %u(%s) flagged inappropriately %x",
                                method_index,
                                GetMethodDescriptionOrError(begin_, header_, method_index).c_str(),
                                method_access_flags);
      return false;
