...
namespace dex_ir{
    ...
    template<class T> bool VerifyIds(dex_ir::CollectionVector<T>& orig,
        ...
        if (orig_iter != orig.end() || output_iter != output.end()) {
            ...
            *error_msg = base::StringPrintf("Mismatch for %s section: %s is longer.", section_name, longer);
            return false;
        }
        return true;
    }
    ...
    bool VerifyId(dex_ir::StringId* orig, dex_ir::StringId* output, std::string* error_msg) {
        if (strcmp(orig->Data(), output->Data()) != 0) {
            *error_msg = base::StringPrintf("Mismatched string data for string id %u at offset %x: %s vs %s.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig->Data(),
                                      output->Data());
    ...
    bool VerifyId(dex_ir::TypeId* orig, dex_ir::TypeId* output, std::string* error_msg) {
        if (orig->GetStringId()->GetIndex() != output->GetStringId()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched string index for type id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
    ...
    bool VerifyId(dex_ir::ProtoId* orig, dex_ir::ProtoId* output, std::string* error_msg) {
        if (orig->Shorty()->GetIndex() != output->Shorty()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched string index for proto id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
        ...
        if (orig->ReturnType()->GetIndex() != output->ReturnType()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched type index for proto id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
        ...
        if (!VerifyTypeList(orig->Parameters(), output->Parameters())) {
            *error_msg =base::StringPrintf("Mismatched type list for proto id %u at offset %x.",
                                      orig->GetIndex(),
                                      orig->GetOffset());
        }
        return true;
    ...
    bool VerifyId(dex_ir::FieldId* orig, dex_ir::FieldId* output, std::string* error_msg) {
        if (orig->Class()->GetIndex() != output->Class()->GetIndex()) {
            *error_msg =
                    base::StringPrintf("Mismatched class type index for field id %u at offset %x: %u vs %u.",
                                 orig->GetIndex(),
                                 orig->GetOffset(),
                                 orig->Class()->GetIndex(),
        ...
        if (orig->Type()->GetIndex() != output->Type()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched type index for field id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig->Class()->GetIndex(),
                                      output->Class()->GetIndex());
        ...
        if (orig->Name()->GetIndex() != output->Name()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched string index for field id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig->Name()->GetIndex(),
    ...
    bool VerifyId(dex_ir::MethodId* orig, dex_ir::MethodId* output, std::string* error_msg) {
        if (orig->Class()->GetIndex() != output->Class()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched type index for method id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig->Class()->GetIndex(),
                                      output->Class()->GetIndex());
        ...
        if (orig->Proto()->GetIndex() != output->Proto()->GetIndex()) {
            *error_msg = base::StringPrintf("Mismatched proto index for method id %u at offset %x: %u vs %u.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig->Class()->GetIndex(),
                                      output->Class()->GetIndex());
        ...
        if (orig->Name()->GetIndex() != output->Name()->GetIndex()) {
            *error_msg =
                    base::StringPrintf("Mismatched string index for method id %u at offset %x: %u vs %u.",
                                 orig->GetIndex(),
                                 orig->GetOffset(),
                                 orig->Name()->GetIndex(),
                                 output->Name()->GetIndex());
...
// identify them and sort them for comparison.
    bool VerifyClassDefs(dex_ir::CollectionVector<dex_ir::ClassDef>& orig,
        ...
        if (orig_iter != orig.end() || output_iter != output.end()) {
            ...
            *error_msg = base::StringPrintf("Mismatch for class defs section: %s is longer.", longer);
            return false;
        }
    ...
    bool VerifyClassDef(dex_ir::ClassDef* orig, dex_ir::ClassDef* output, std::string* error_msg) {
        if (orig->ClassType()->GetIndex() != output->ClassType()->GetIndex()) {
            *error_msg =
                    base::StringPrintf("Mismatched class type index for class def %u at offset %x: %u vs %u.",
                                 orig->GetIndex(),
                                 orig->GetOffset(),
        ...
        if (orig->GetAccessFlags() != output->GetAccessFlags()) {
            *error_msg =
                    base::StringPrintf("Mismatched access flags for class def %u at offset %x: %x vs %x.",
                                 orig->GetIndex(),
                                 orig->GetOffset(),
                                 orig->GetAccessFlags(),
        ...
        if (orig_super != output_super) {
            *error_msg =
                    base::StringPrintf("Mismatched super class for class def %u at offset %x: %u vs %u.",
                                 orig->GetIndex(),
                                 orig->GetOffset(),
                                 orig_super,
                                 output_super);
        ...
        if (!VerifyTypeList(orig->Interfaces(), output->Interfaces())) {
            *error_msg = base::StringPrintf("Mismatched type list for class def %u at offset %x.",
                                      orig->GetIndex(),
                                      orig->GetOffset());
            return false;
        }
        ...
        if (strcmp(orig_source, output_source) != 0) {
            *error_msg = base::StringPrintf("Mismatched source file for class def %u at offset %x: %s vs %s.",
                                      orig->GetIndex(),
                                      orig->GetOffset(),
                                      orig_source,
                                      output_source);
    ...
    bool VerifyAnnotationSet(dex_ir::AnnotationSetItem* orig,
        ...
        if (orig_items->size() != output_items->size()) {
            *error_msg = base::StringPrintf("Mismatched size for annotation set at offset %x: %zu vs %zu.",
                                      orig->GetOffset(),
                                      orig_items->size(),
                                      output_items->size());
            return false;
    ...
    bool VerifyAnnotation(dex_ir::AnnotationItem* orig,
        ...
        if (orig->GetVisibility() != output->GetVisibility()) {
            *error_msg = base::StringPrintf("Mismatched visibility for annotation at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->GetVisibility(),
        ...
        switch (orig->Type()) {
            case libdex::DexFile::kDexAnnotationByte:
                if (orig->GetByte() != output->GetByte()) {
                    *error_msg = base::StringPrintf("Mismatched encoded byte for annotation at offset %x: %d vs %d.",
                                              orig_offset,
                                              orig->GetByte(),
                                              output->GetByte());
                    return false;
            ...
            case libdex::DexFile::kDexAnnotationShort:
                if (orig->GetShort() != output->GetShort()) {
                    *error_msg = base::StringPrintf("Mismatched encoded short for annotation at offset %x: %d vs %d.",
                                              orig_offset,
                                              orig->GetShort(),
                                              output->GetShort());
                    return false;
            ...
            case libdex::DexFile::kDexAnnotationChar:
                if (orig->GetChar() != output->GetChar()) {
                    *error_msg = base::StringPrintf("Mismatched encoded char for annotation at offset %x: %c vs %c.",
                                              orig_offset,
                                              orig->GetChar(),
                                              output->GetChar());
                    return false;
            ...
            case libdex::DexFile::kDexAnnotationInt:
                if (orig->GetInt() != output->GetInt()) {
                    *error_msg = base::StringPrintf("Mismatched encoded int for annotation at offset %x: %d vs %d.",
                                              orig_offset,
                                              orig->GetInt(),
                                              output->GetInt());
                    return false;
                ...
                break;
            case libdex::DexFile::kDexAnnotationLong:
                ...
                break;
            case libdex::DexFile::kDexAnnotationFloat:
                ...
                break;
            case libdex::DexFile::kDexAnnotationDouble:
                ...
                break;
            case libdex::DexFile::kDexAnnotationString:
                if (orig->GetStringId()->GetIndex() != output->GetStringId()->GetIndex()) {
                    *error_msg = base::StringPrintf(
                ...
                break;
            case libdex::DexFile::kDexAnnotationType:
                if (orig->GetTypeId()->GetIndex() != output->GetTypeId()->GetIndex()) {
                    *error_msg = base::StringPrintf("Mismatched encoded type for annotation at offset %x: %u vs %u.",
                                              orig_offset,
                ...
                break;
            case libdex::DexFile::kDexAnnotationField:
            case libdex::DexFile::kDexAnnotationEnum:
                if (orig->GetFieldId()->GetIndex() != output->GetFieldId()->GetIndex()) {
                    *error_msg = base::StringPrintf("Mismatched encoded field for annotation at offset %x: %u vs %u.",
                                              orig_offset,
                ...
                break;
            case libdex::DexFile::kDexAnnotationMethod:
                ...
                break;
            case libdex::DexFile::kDexAnnotationArray:
                if (!VerifyEncodedArray(orig->GetEncodedArray(), output->GetEncodedArray(), error_msg)) {
                    return false;
                }
                break;
            case libdex::DexFile::kDexAnnotationAnnotation:
                ...
                break;
            case libdex::DexFile::kDexAnnotationNull:
                break;
            case libdex::DexFile::kDexAnnotationBoolean:
    ...
    bool VerifyEncodedArray(dex_ir::EncodedArrayItem* orig,
        ...
        if (orig_vector->size() != output_vector->size()) {
            *error_msg = base::StringPrintf("Mismatched size for encoded array at offset %x: %zu vs %zu.",
                                      orig->GetOffset(),
                                      orig_vector->size(),
                                      output_vector->size());
            return false;
    ...
    bool VerifyFields(dex_ir::FieldItemVector* orig,
                      ...
                      std::string* error_msg) {
        if (orig->size() != output->size()) {
            *error_msg = base::StringPrintf("Mismatched fields size for class data at offset %x: %zu vs %zu.",
                                      orig_offset,
                                      orig->size(),
                                      output->size());
            return false;
        ...
        for (size_t i = 0; i < orig->size(); ++i) {
          /*  dex_ir::FieldItem* orig_field = &(orig)[i];
            ...
            if (orig_field->GetFieldId()->GetIndex() != output_field->GetFieldId()->GetIndex()) {
                *error_msg = base::StringPrintf("Mismatched field index for class data at offset %x: %u vs %u.",
                                          orig_offset,
                                          orig_field->GetFieldId()->GetIndex(),
                                          output_field->GetFieldId()->GetIndex());
                return false;
    ...
    bool VerifyMethods(dex_ir::MethodItemVector* orig,
                       ...
                       std::string* error_msg) {
        if (orig->size() != output->size()) {
            *error_msg = base::StringPrintf("Mismatched methods size for class data at offset %x: %zu vs %zu.",
                                      orig_offset,
                                      orig->size(),
                                      output->size());
            return false;
        ...
        for (size_t i = 0; i < orig->size(); ++i) {
            ...
            if (orig_method->GetMethodId()->GetIndex() != output_method->GetMethodId()->GetIndex()) {
                *error_msg = base::StringPrintf("Mismatched method index for class data at offset %x: %u vs %u.",
                                          orig_offset,
                                          orig_method->GetMethodId()->GetIndex(),
                                          output_method->GetMethodId()->GetIndex());
                return false;
    ...
    bool VerifyCode(dex_ir::CodeItem* orig, dex_ir::CodeItem* output, std::string* error_msg) {
        ...
        if (orig->RegistersSize() != output->RegistersSize()) {
            *error_msg = base::StringPrintf("Mismatched registers size for code item at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->RegistersSize(),
        ...
        if (orig->InsSize() != output->InsSize()) {
            *error_msg = base::StringPrintf("Mismatched ins size for code item at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->InsSize(),
                                      output->InsSize());
            return false;
        ...
        if (orig->OutsSize() != output->OutsSize()) {
            *error_msg = base::StringPrintf("Mismatched outs size for code item at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->OutsSize(),
                                      output->OutsSize());
            return false;
        ...
        if (orig->TriesSize() != output->TriesSize()) {
            *error_msg = base::StringPrintf("Mismatched tries size for code item at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->TriesSize(),
                                      output->TriesSize());
            return false;
        ...
        if (orig->InsnsSize() != output->InsnsSize()) {
            *error_msg = base::StringPrintf("Mismatched insns size for code item at offset %x: %u vs %u.",
                                      orig->GetOffset(),
                                      orig->InsnsSize(),
                                      output->InsnsSize());
            return false;
        ...
        if (memcmp(orig->Insns(), output->Insns(), orig->InsnsSize()) != 0) {
            *error_msg = base::StringPrintf("Mismatched insns for code item at offset %x.",
                                      orig->GetOffset());
            return false;
        }
    ...
    bool VerifyTries(dex_ir::TryItemVector* orig,
        ...
        if (orig->size() != output->size()) {
            *error_msg = base::StringPrintf("Mismatched tries size for code item at offset %x: %zu vs %zu.",
                                      orig_offset,
                                      orig->size(),
                                      output->size());
            return false;
