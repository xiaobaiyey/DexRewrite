 ...
 */
    ...
    public:
        ...
        RegisterValue(RegisterSource source, libdex::DexFileReference reference, const VeriClass *type)
                : source_(source), value_(0), reference_(reference), type_(type) {}

        RegisterValue(RegisterSource source,
                      uint32_t value,
                      libdex::DexFileReference reference,
        ...
        libdex::DexFileReference GetDexFileReference() const { return reference_; }
        ...
        std::string ToString() const {
            switch (source_) {
                case RegisterSource::kString: {
                    const char *str = reference_.dex_file->StringDataByIdx(libdex::dex::StringIndex(reference_.index));
                ...
                case RegisterSource::kClass:
                    return reference_.dex_file->StringByTypeIdx(libdex::dex::TypeIndex(reference_.index));
    ...
    private:
        ...
        uint32_t value_;
        libdex::DexFileReference reference_;
    ...
    public:
        VeriFlowAnalysis(VeridexResolver *resolver, const libdex::ClassAccessor::Method &method);

        void Run();

        virtual RegisterValue AnalyzeInvoke(const libdex::Instruction &instruction, bool is_range) = 0;

        virtual void AnalyzeFieldSet(const libdex::Instruction &instruction) = 0;
    ...
    private:
        ...
        void ProcessDexInstruction(const libdex::Instruction &inst);

        void SetVisited(uint32_t dex_pc);

        RegisterValue GetFieldType(uint32_t field_index);

        int GetBranchFlags(const libdex::Instruction &instruction) const;
    ...
    private:
        ...
        libdex::CodeItemDataAccessor code_item_accessor_;
...
// Collects all reflection uses.
    ...
    public:
        FlowAnalysisCollector(VeridexResolver *resolver, const libdex::ClassAccessor::Method &method)
        ...
        RegisterValue AnalyzeInvoke(const libdex::Instruction &instruction, bool is_range) override;

        void AnalyzeFieldSet(const libdex::Instruction &instruction) override;
...
// Substitutes reflection uses by new ones.
    ...
    public:
        FlowAnalysisSubstitutor(VeridexResolver *resolver,
                                ...
                                const std::map<libdex::MethodReference, std::vector<ReflectAccessInfo>> &accesses)
    ...
    private:
        ...
        const std::map<libdex::MethodReference, std::vector<ReflectAccessInfo>> &accesses_;
    };
