...
class PreciseHiddenApiFinder {
 public:
  ...
  void Dump(std::ostream& os, HiddenApiStats* stats, std::map<std::string, std::vector<std::string>> &greylist,
 ...
 private:
  ...
  void AddUsesAt(const std::vector<ReflectAccessInfo>& accesses, libdex::MethodReference ref);

  const HiddenApi& hidden_api_;

  std::map<libdex::MethodReference, std::vector<ReflectAccessInfo>> concrete_uses_;
  std::map<libdex::MethodReference, std::vector<ReflectAccessInfo>> abstract_uses_;
};
