...
class VeridexResolver {
 public:
  VeridexResolver(const  libdex::DexFile& dex_file,
  ...
  VeriClass* GetVeriClass( libdex::dex::TypeIndex index);
  ...
  const libdex::DexFile& GetDexFile() const {
    return dex_file_;
  }

  const libdex::DexFile& GetDexFileOf(const VeriClass& kls) {
 ...
 private:
  ...
  const libdex::DexFile& dex_file_;
  TypeMap& type_map_;
