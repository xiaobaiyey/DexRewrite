...
#ifndef WAAPOLICY_DEX_FILE_H
#define WAAPOLICY_DEX_FILE_H

#include <base/macros.h>
#include "dex_types.h"
#include "dex_modifiers.h"
#include "dex_stringpiece.h"
#include <vector>
...
// such management do so by subclassing Container.
    ...
    public:
        ...
        // Raw type_id_item.
        struct TypeId {
            StringIndex descriptor_idx_;  // index into string_ids
        ...
        // Raw field_id_item.
        struct FieldId {
            TypeIndex class_idx_;   // index into type_ids_ array for defining class
            TypeIndex type_idx_;    // index into type_ids_ array for field type
            StringIndex name_idx_;  // index into string_ids_ array for field name
        ...
        // Raw proto_id_item.
        struct ProtoId {
            StringIndex shorty_idx_;     // index into string_ids array for shorty descriptor
            TypeIndex return_type_idx_;  // index into type_ids array for return type
        ...
        struct MethodId {
            ...
            StringIndex name_idx_;  // index into string_ids_ array for method name
        ...
        // Raw class_def_item.
        struct ClassDef {
            TypeIndex class_idx_;  // index into type_ids_ array for this class
            uint16_t pad1_;  // padding = 0
            uint32_t access_flags_;
            TypeIndex superclass_idx_;  // index into type_ids_ array for superclass
            ...
            uint32_t GetJavaAccessFlags() const {
                ...
                static_assert(
                        (kAccValidInterfaceFlags & kAccJavaFlagsMask) == kAccValidInterfaceFlags,
        ...
        // Raw type_item.
        struct TypeItem {
            TypeIndex type_idx_;  // index into type_ids section
        ...
        const char *
        GetStringDataAndUtf16Length(const StringId &string_id, uint32_t *utf16_length) const;

        const char *GetStringData(const StringId &string_id) const;

        // Index version of GetStringDataAndUtf16Length.
        const char *
        StringDataAndUtf16LengthByIdx(StringIndex idx, uint32_t *utf16_length) const;

        const char *StringDataByIdx(StringIndex idx) const;
        ...
        bool IsTypeIndexValid(TypeIndex idx) const {
            return idx.IsValid() && idx.index_ < NumTypeIds();
        }

        // Returns the TypeId at the specified index.
        const TypeId &GetTypeId(TypeIndex idx) const {
            DCHECK_LT(idx.index_, NumTypeIds()) << GetLocation();
            return type_ids_[idx.index_];
        }

        TypeIndex GetIndexForTypeId(const TypeId &type_id) const {
            ...
            return TypeIndex(static_cast<uint16_t>(result));
        }

        // Get the descriptor string associated with a given type index.
        const char *StringByTypeIdx(TypeIndex idx, uint32_t *unicode_length) const;

        const char *StringByTypeIdx(TypeIndex idx) const;

        // Returns the type descriptor string of a type id.
        const char *GetTypeDescriptor(const TypeId &type_id) const;

        // Looks up a type for the given string index
        const TypeId *FindTypeId(StringIndex string_idx) const;
        ...
        const ClassDef *FindClassDef(TypeIndex type_idx) const;
        ...
        const ProtoId *FindProtoId(TypeIndex return_type_idx,
                                   const TypeIndex *signature_type_idxs,
                                   uint32_t signature_length) const;

        const ProtoId *FindProtoId(TypeIndex return_type_idx, const std::vector<TypeIndex> &signature_type_idxs) const {
            return FindProtoId(return_type_idx, &signature_type_idxs[0],
                               signature_type_idxs.size());
        }
        ...
        GetTryItems(const DexInstructionIterator &code_item_end, uint32_t offset);
        ...
        const AnnotationSetItem *
        GetClassAnnotationSet(const AnnotationsDirectoryItem *anno_dir) const {
            return DataPointer<AnnotationSetItem>(anno_dir->class_annotations_off_);
        }

        const FieldAnnotationsItem *
        GetFieldAnnotations(const AnnotationsDirectoryItem *anno_dir) const {
        ...
        const ParameterAnnotationsItem *
        GetParameterAnnotations(const AnnotationsDirectoryItem *anno_dir)
        const {
        ...
        const AnnotationSetItem *
        GetFieldAnnotationSetItem(const FieldAnnotationsItem &anno_item) const {
        ...
        GetAnnotationItem(const AnnotationSetItem *set_item, uint32_t index) const {
        ...
        std::string PrettyType(TypeIndex type_idx) const;
    ...
    public:
        static DexFile *getDexFile(std::string path);

        static DexFile *getDexFile(const uint8_t *base,
                                   size_t size);
...
// Iterate over a dex file's ProtoId's paramters
    ...
    public:
        ...
        TypeIndex GetTypeIdx() {
            return type_list_->GetTypeItem(pos_).type_idx_;
        }

        const char *GetDescriptor() {
            return dex_file_.StringByTypeIdx(TypeIndex(GetTypeIdx()));
        }
...
// Abstract the signature of a method.
    ...
    private:
        Signature(const DexFile *dex, const DexFile::ProtoId &proto) : dex_file_(dex),
                                                                       proto_id_(&proto) {
        }
...
// Iterate and decode class_data_item
    ...
    public:
        ...
        uint32_t GetMemberAccessFlags() const {
            return GetRawMemberAccessFlags();
        }
    ...
    std::ostream &
    operator<<(std::ostream &os, const EncodedStaticFieldValueIterator::ValueType &code);
