...
DexWriter::DexWriter(dex_ir::Header *header, bool compute_offsets)
        : header_(header),
...
void DexWriter::WriteStringIds(Stream *stream, bool reserve_only) {
    ...
    for (std::unique_ptr<dex_ir::StringId> &string_id : header_->GetCollections().StringIds()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeStringIdItem));
        if (reserve_only) {
...
void DexWriter::WriteStringData(Stream *stream, dex_ir::StringData *string_data) {
    ...
    stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeStringDataItem));
...
void DexWriter::WriteTypeIds(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::TypeId> &type_id : header_->GetCollections().TypeIds()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeTypeIdItem));
...
void DexWriter::WriteTypeLists(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::TypeList> &type_list : header_->GetCollections().TypeLists()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeTypeList));
...
void DexWriter::WriteProtoIds(Stream *stream, bool reserve_only) {
    ...
    for (std::unique_ptr<dex_ir::ProtoId> &proto_id : header_->GetCollections().ProtoIds()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeProtoIdItem));
...
void DexWriter::WriteFieldIds(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::FieldId> &field_id : header_->GetCollections().FieldIds()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeFieldIdItem));
...
void DexWriter::WriteMethodIds(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::MethodId> &method_id : header_->GetCollections().MethodIds()) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeMethodIdItem));
...
void DexWriter::WriteEncodedArrays(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::EncodedArrayItem> &encoded_array :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeEncodedArrayItem));
...
void DexWriter::WriteAnnotations(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::AnnotationItem> &annotation :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeAnnotationItem));
...
void DexWriter::WriteAnnotationSets(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::AnnotationSetItem> &annotation_set :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeAnnotationSetItem));
...
void DexWriter::WriteAnnotationSetRefs(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::AnnotationSetRefList> &annotation_set_ref :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeAnnotationSetRefList));
...
void DexWriter::WriteAnnotationsDirectories(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::AnnotationsDirectoryItem> &annotations_directory :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeAnnotationsDirectoryItem));
...
void DexWriter::WriteDebugInfoItem(Stream *stream, dex_ir::DebugInfoItem *debug_info) {
    stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeDebugInfoItem));
...
void DexWriter::WriteCodeItemPostInstructionData(Stream *stream,
    ...
    if (code_item->TriesSize() != 0) {
        stream->AlignTo(art::DexFile::TryItem::kAlignment);
        // Write try items.
        for (std::unique_ptr<const dex_ir::TryItem> &try_item : *code_item->Tries()) {
            art::DexFile::TryItem disk_try_item;
            if (!reserve_only) {
...
void DexWriter::WriteCodeItem(Stream *stream,
    ...
    stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeCodeItem));
    ProcessOffset(stream, code_item);

    art::StandardDexFile::CodeItem disk_code_item;
    if (!reserve_only) {
    ...
    // item.
    stream->Write(&disk_code_item, OFFSETOF_MEMBER(art::StandardDexFile::CodeItem, insns_));
...
void DexWriter::WriteClassDatas(Stream *stream) {
    ...
    for (const std::unique_ptr<dex_ir::ClassData> &class_data :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeClassDataItem));
...
void DexWriter::WriteCallSiteIds(Stream *stream, bool reserve_only) {
    ...
    for (std::unique_ptr<dex_ir::CallSiteId> &call_site_id :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeCallSiteIdItem));
        if (reserve_only) {
...
void DexWriter::WriteMethodHandles(Stream *stream) {
    ...
    for (std::unique_ptr<dex_ir::MethodHandleItem> &method_handle :
        ...
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeMethodHandleItem));
...
void DexWriter::GenerateAndWriteMapItems(Stream *stream) {
    ...
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeHeaderItem, 1, 0));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeStringIdItem,
                                collection.StringIdsSize(),
                                collection.StringIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeTypeIdItem,
                                collection.TypeIdsSize(),
                                collection.TypeIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeProtoIdItem,
                                collection.ProtoIdsSize(),
                                collection.ProtoIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeFieldIdItem,
                                collection.FieldIdsSize(),
                                collection.FieldIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeMethodIdItem,
                                collection.MethodIdsSize(),
                                collection.MethodIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeClassDefItem,
                                collection.ClassDefsSize(),
                                collection.ClassDefsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeCallSiteIdItem,
                                collection.CallSiteIdsSize(),
                                collection.CallSiteIdsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeMethodHandleItem,
                                collection.MethodHandleItemsSize(),
                                collection.MethodHandleItemsOffset()));
    // Data section.
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeMapList, 1, collection.MapListOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeTypeList,
                                collection.TypeListsSize(),
                                collection.TypeListsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeAnnotationSetRefList,
                                collection.AnnotationSetRefListsSize(),
                                collection.AnnotationSetRefListsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeAnnotationSetItem,
                                collection.AnnotationSetItemsSize(),
                                collection.AnnotationSetItemsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeClassDataItem,
                                collection.ClassDatasSize(),
                                collection.ClassDatasOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeCodeItem,
                                collection.CodeItemsSize(),
                                collection.CodeItemsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeStringDataItem,
                                collection.StringDatasSize(),
                                collection.StringDatasOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeDebugInfoItem,
                                collection.DebugInfoItemsSize(),
                                collection.DebugInfoItemsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeAnnotationItem,
                                collection.AnnotationItemsSize(),
                                collection.AnnotationItemsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeEncodedArrayItem,
                                collection.EncodedArrayItemsSize(),
                                collection.EncodedArrayItemsOffset()));
    queue.AddIfNotEmpty(MapItem(art::DexFile::kDexTypeAnnotationsDirectoryItem,
...
void DexWriter::WriteHeader(Stream *stream) {
    art::StandardDexFile::Header header;
    if (art::StandardDexFile::IsMagicValid(header_->Magic())) {
        art::StandardDexFile::WriteMagic(header.magic_);
        // TODO: Should we write older versions based on the feature flags?
        art::StandardDexFile::WriteCurrentVersion(header.magic_);
    } else {
        // Standard dex -> standard dex, just reuse the same header.
        static constexpr size_t kMagicAndVersionLen =
                art::StandardDexFile::kDexMagicSize + art::StandardDexFile::kDexVersionLen;
        std::copy_n(header_->Magic(), kMagicAndVersionLen, header.magic_);
    }
    header.checksum_ = header_->Checksum();
    std::copy_n(header_->Signature(), art::DexFile::kSha1DigestSize, header.signature_);
...
size_t DexWriter::GetHeaderSize() const {
    return sizeof(art::StandardDexFile::Header);
}

bool DexWriter::Write(art::DexContainer *output, std::string *error_msg) {
    ...
    // Write the map list.
    if (compute_offsets_) {
        stream->AlignTo(SectionAlignment(art::DexFile::kDexTypeMapList));
    ...
    header_->SetChecksum(art::DexFile::CalculateChecksum(stream->Begin(), header_->FileSize()));
    ...
    __android_log_print(ANDROID_LOG_ERROR, __FUNCTION__, "[+] MainSection:0x%zx",
...
bool DexWriter::Output(dex_ir::Header *header,
                       std::string outpath) {
    std::unique_ptr<DexWriter> writer;

    writer.reset(new DexWriter(header, true));
    std::unique_ptr<art::DexContainer> container = writer->CreateDexContainer();;
    std::string error_msg;
    bool write_ = writer->Write(container.get(), &error_msg);
    if (FdFile::FileExists(outpath.c_str(), false)) {
        FdFile::DeleteFile(outpath.c_str());
    }
    FdFile *file = FdFile::CreateEmptyFile(outpath.c_str());
    uint8_t *data = container.get()->GetMainSection()->Begin();
    if (data == nullptr)
        return false;
    uint32_t len = container.get()->GetMainSection()->Size();
    CHECK_NE(len, 0);
    bool write_result = file->WriteFully(data, len);
    CHECK_EQ(write_result, true);
    int close_result = file->FlushCloseOrErase();
...
std::unique_ptr<art::DexContainer> DexWriter::CreateDexContainer() const {
    return std::unique_ptr<art::DexContainer>(new DexWriter::Container);
}
